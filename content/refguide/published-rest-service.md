...
* Select **Microflow** to create a custom authentication microflow. This microflow is called every time a user want to access a resource.
  ...
  * In case of an empty user, one of the following action will be performed:
      1. If the status code of the HTTP Response is different from 200 OK, the response is returned to the client
      2. If the status code of the HTTP Response is 200 OK, the next authentication method is invoked (Basic authentication)
